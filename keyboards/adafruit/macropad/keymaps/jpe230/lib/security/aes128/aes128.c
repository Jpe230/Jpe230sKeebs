...
static state_t* aes_state;

static uint8_t round_key[keyExpSize];
...
static void add_round_key(uint8_t round) {
    ...
    for(i=0;i<4;++i)
    {
        for(j = 0; j < 4; ++j)
        {
            (*aes_state)[i][j] ^= round_key[round * Nb * 4 + i * Nb + j];
        }
    }
}
...
static void sub_bytes(void) {
  ...
  for(i = 0; i < 4; ++i)
  {
    for(j = 0; j < 4; ++j)
    {
      (*aes_state)[j][i] = get_sbox_value((*aes_state)[j][i]);
    }
  }
}
...
static void mix_columns(void) {
  ...
  {  
    ...
    Tm  = (*aes_state)[i][0] ^ (*aes_state)[i][1] ; Tm = xtime(Tm);  (*aes_state)[i][0] ^= Tm ^ Tmp ;
    Tm  = (*aes_state)[i][1] ^ (*aes_state)[i][2] ; Tm = xtime(Tm);  (*aes_state)[i][1] ^= Tm ^ Tmp ;
    Tm  = (*aes_state)[i][2] ^ (*aes_state)[i][3] ; Tm = xtime(Tm);  (*aes_state)[i][2] ^= Tm ^ Tmp ;
    Tm  = (*aes_state)[i][3] ^ t ;                  Tm = xtime(Tm);  (*aes_state)[i][3] ^= Tm ^ Tmp ;
  }
}
...
static void inv_mix_columns(void) {
    ...
    { 
        ...
        (*aes_state)[i][0] = multiply(a, 0x0e) ^ multiply(b, 0x0b) ^ multiply(c, 0x0d) ^ multiply(d, 0x09);
        (*aes_state)[i][1] = multiply(a, 0x09) ^ multiply(b, 0x0e) ^ multiply(c, 0x0b) ^ multiply(d, 0x0d);
        (*aes_state)[i][2] = multiply(a, 0x0d) ^ multiply(b, 0x09) ^ multiply(c, 0x0e) ^ multiply(d, 0x0b);
        (*aes_state)[i][3] = multiply(a, 0x0b) ^ multiply(b, 0x0d) ^ multiply(c, 0x09) ^ multiply(d, 0x0e);
    }
}
...
static void inv_sub_bytes(void) {
    ...
    for(i=0;i<4;++i)
    {
        for(j=0;j<4;++j)
        {
            (*aes_state)[j][i] = get_sbox_invert((*aes_state)[j][i]);
        }
    }
}
...
void AES_ECB_decrypt(const uint8_t* input, const uint8_t* _key, uint8_t *output, const uint32_t length) {
...
void AES_ECB_encrypt(const uint8_t* input, const uint8_t* _key, uint8_t* output, const uint32_t length) {
  // Copy input to output, and work in-memory on output
  memcpy(output, input, length);
  aes_state = (state_t*)output;
