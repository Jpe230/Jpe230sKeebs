...
void sha256_final(SHA256_CTX *ctx, char hash[]) {
	...
	// reverse all the char when copying the final state to the output hash.
	for (i = 0; i < 4; ++i) {
		hash[i]      = (ctx->sha256_state[0] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 4]  = (ctx->sha256_state[1] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 8]  = (ctx->sha256_state[2] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 12] = (ctx->sha256_state[3] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 16] = (ctx->sha256_state[4] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 20] = (ctx->sha256_state[5] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 24] = (ctx->sha256_state[6] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 28] = (ctx->sha256_state[7] >> (24 - i * 8)) & 0x000000ff;
	}
}