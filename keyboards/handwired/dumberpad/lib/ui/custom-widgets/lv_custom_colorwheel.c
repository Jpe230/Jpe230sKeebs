 ...
 */
  ...
  *      INCLUDES
  *********************/
#include "lv_custom_colorwheel.h"
#if LV_USE_CUSTOM_COLORWHEEL

#if __has_include("lvgl/src/misc/lv_assert.h")
...
#include "src/misc/lv_assert.h"
   ...
   *      DEFINES
   *********************/
#define MY_CLASS &lv_custom_colorwheel_class
...
#define OUTER_MASK_WIDTH 3
     ...
     **********************/
      ...
      *  STATIC PROTOTYPES
      **********************/
static void lv_custom_colorwheel_constructor(const lv_obj_class_t* class_p, lv_obj_t* obj);
static void lv_custom_colorwheel_event(const lv_obj_class_t* class_p, lv_event_t* e);
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_custom_colorwheel_class = { .instance_size = sizeof(lv_custom_colorwheel_t), .base_class = &lv_obj_class,
                                            .constructor_cb = lv_custom_colorwheel_constructor,
                                            .event_cb = lv_custom_colorwheel_event,
 ...
 **********************/
  ...
  **********************/
   ...
   */
lv_obj_t* lv_custom_colorwheel_create(lv_obj_t* parent, bool knob_recolor, uint8_t scale)
{
    LV_LOG_INFO("begin");
 ...
 *====================*/
  ...
  */
bool lv_custom_colorwheel_set_hsv(lv_obj_t* obj, lv_color_hsv_t hsv)
{
    ...
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
 ...
 * @return true if changed, otherwise false
 */
bool lv_custom_colorwheel_set_rgb(lv_obj_t* obj, lv_color_t color)
{
    lv_color32_t c32;
    c32.full = lv_color_to32(color);

    return lv_custom_colorwheel_set_hsv(obj, lv_color_rgb_to_hsv(c32.ch.red, c32.ch.green, c32.ch.blue));
}
 ...
 */
void lv_custom_colorwheel_set_mode(lv_obj_t* obj, lv_custom_colorwheel_mode_t mode)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
 ...
 */
void lv_custom_colorwheel_set_mode_fixed(lv_obj_t* obj, bool fixed)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
 ...
 *====================*/
  ...
  */
lv_color_hsv_t lv_custom_colorwheel_get_hsv(lv_obj_t* obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
 ...
 */
lv_color_t lv_custom_colorwheel_get_rgb(lv_obj_t* obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;

    uint16_t t_s = ((uint32_t)colorwheel->hsv.s * 100 / 255);
    uint16_t t_v = ((uint32_t)colorwheel->hsv.v * 100 / 255);

    return lv_color_hsv_to_rgb(
        (colorwheel->hsv.h * 360) >> 8 ,
        (uint8_t)t_s,
        (uint8_t)t_v
    );
}
 ...
 */
lv_custom_colorwheel_mode_t lv_custom_colorwheel_get_color_mode(lv_obj_t* obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
 ...
 */
bool lv_custom_colorwheel_get_color_mode_fixed(lv_obj_t* obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
...
static void lv_custom_colorwheel_constructor(const lv_obj_class_t* class_p, lv_obj_t* obj)
{
    ...
    lv_custom_colorwheel_t* colorwheel = (lv_custom_colorwheel_t*)obj;
    colorwheel->hsv.h = 0;
    colorwheel->hsv.s = 255;
    colorwheel->hsv.v = 255;
    colorwheel->mode = LV_CUSTOM_COLORWHEEL_MODE_HUE;
...
static void draw_disc_grad(lv_event_t* e)
{
    ...
    for (i = 0; i <= 256; i += LV_CPICKER_DEF_QF, a += 360 * LV_CPICKER_DEF_QF) {
        ...
        angle_trigo = (uint16_t)map(angle_trigo, 0, 360, 285, 75);
...
static void draw_knob(lv_event_t* e)
{
    ...
    if (colorwheel->knob.recolor) {
        cir_dsc.bg_color = lv_custom_colorwheel_get_rgb(obj);
    }
    ...
    if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        *s = LV_MAX(*s, knob_pad);
        ...
        if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
            ...
            case LV_CUSTOM_COLORWHEEL_MODE_HUE:
                hsv_cur.h = (colorwheel->hsv.h + (1 * use_scale)) % 255;
                break;
            case LV_CUSTOM_COLORWHEEL_MODE_SATURATION:
                hsv_cur.s = (colorwheel->hsv.s + (1 * use_scale)) % 255;
                break;
            case LV_CUSTOM_COLORWHEEL_MODE_VALUE:
                hsv_cur.v = (colorwheel->hsv.v + (1 * use_scale)) % 255;
                break;
            }

            if (lv_custom_colorwheel_set_hsv(obj, hsv_cur)) {
            ...
            switch (colorwheel->mode) {
            case LV_CUSTOM_COLORWHEEL_MODE_HUE:
                hsv_cur.h = colorwheel->hsv.h > 0 ? (colorwheel->hsv.h - (1 * use_scale)) : 255;
                break;
            case LV_CUSTOM_COLORWHEEL_MODE_SATURATION:
                hsv_cur.s = colorwheel->hsv.s > 0 ? (colorwheel->hsv.s - (1 * use_scale)) : 255;
                break;
            case LV_CUSTOM_COLORWHEEL_MODE_VALUE:
                hsv_cur.v = colorwheel->hsv.v > 0 ? (colorwheel->hsv.v - (1 * use_scale)) : 255;
                break;
            }
        ...
        case LV_CUSTOM_COLORWHEEL_MODE_HUE:
            ...
            break;
        case LV_CUSTOM_COLORWHEEL_MODE_SATURATION:
            hsv_cur.s = (angle * 255) / 360;
            break;
        case LV_CUSTOM_COLORWHEEL_MODE_VALUE:
            hsv_cur.v = (angle * 255) / 360;
            break;
        }
...
static void refr_knob_pos(lv_obj_t* obj)
{
    ...
    angle = (uint16_t)map(angle, 0, 360, 285, 75);
...
static lv_color_t angle_to_mode_color_fast(lv_obj_t* obj, uint16_t angle)
{
    lv_custom_colorwheel_t* ext = (lv_custom_colorwheel_t*)obj;
    uint8_t r = 0, g = 0, b = 0;
    static uint16_t h = 0;
    ...
    case LV_CUSTOM_COLORWHEEL_MODE_HUE:
        ...
        if (m != ext->mode) {
            s = (uint8_t)ext->hsv.s;//(uint8_t)(((uint16_t)ext->hsv.s * 51) / 20);
            v = (uint8_t)ext->hsv.v;//(uint8_t)(((uint16_t)ext->hsv.v * 51) / 20);
            m = ext->mode;
    ...
    case LV_CUSTOM_COLORWHEEL_MODE_SATURATION:
        /*Don't recompute costly scaling if it does not change*/
        if (m != ext->mode) {
            h = (uint16_t)((uint32_t)ext->hsv.h * 6);
            v = (uint8_t)ext->hsv.v;
            m = ext->mode;
        }
...
static uint16_t get_angle(lv_obj_t* obj)
{
    ...
    default:
    case LV_CUSTOM_COLORWHEEL_MODE_HUE:
        angle = (colorwheel->hsv.h * 360) / 255;
        break;
    case LV_CUSTOM_COLORWHEEL_MODE_SATURATION:
        angle = (colorwheel->hsv.s * 360) / 255;
        break;
    case LV_CUSTOM_COLORWHEEL_MODE_VALUE:
        angle = (colorwheel->hsv.v * 360) / 255;
        break;
    }
    return angle;
}
